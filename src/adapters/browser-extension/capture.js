...
async function captureBrowserExtension({projectDir, driver, options = {}}) {
  ...
  const discovery = discoverBrowserExtensionProject(projectDir);
  const startedAt = new Date().toISOString();
  const raw = await driver.capture({extensionPath:discovery.extension_root,options});
  const env = {...(raw.environment || {}),manifest_version:discovery.manifest.manifest_version};
  const observations = [];
  ...
  if (discovery.manifest.service_worker) add({id:"extension-service-worker-active",kind:"state",source:"extension-service-worker",value:Array.isArray(raw.serviceWorkers)&&raw.serviceWorkers.length>0,evidence:[evidence("service-worker","browser-context",`${(raw.serviceWorkers||[]).length} extension service worker(s) observed`,raw.serviceWorkers || [])]});
  ...
  add({id:"extension-errors",kind:"behavior",source:"browser-runtime",value:(raw.errors||[]).length,evidence:(raw.errors||[]).map((item)=>evidence("runtime-error",item.source||"extension",item.message||String(item),item))});
  for (const area of ["local","sync","session"]) {
    const keys = sortedUnique(raw.storageKeys?.[area]);
    add({id:`storage-${area}-keys`,kind:"state",source:`chrome.storage.${area}`,value:keys,evidence:[evidence("storage-keys",`chrome.storage.${area}`,`Captured ${area} storage key topology without values`,keys)]});
  }
  if (discovery.manifest.popup) add({id:"popup-loads",kind:"behavior",source:"extension-page",value:Boolean(raw.popup?.loaded),evidence:[evidence("extension-page",discovery.manifest.popup,raw.popup?.loaded?"Popup loaded":"Popup failed to load",raw.popup||null)]});
  if (discovery.manifest.options) add({id:"options-loads",kind:"behavior",source:"extension-page",value:Boolean(raw.options?.loaded),evidence:[evidence("extension-page",discovery.manifest.options,raw.options?.loaded?"Options page loaded":"Options page failed to load",raw.options||null)]});
  add({id:"dynamic-content-scripts",kind:"state",source:"chrome.scripting",value:(raw.dynamicContentScripts||[]).map((x)=>({id:x.id||null,matches:sortedUnique(x.matches),world:x.world||null})),evidence:[evidence("content-scripts","chrome.scripting.getRegisteredContentScripts","Dynamically registered content scripts",raw.dynamicContentScripts||[])]});
  if (raw.targetPage) add({id:"target-page-loads",kind:"environment",source:"browser",value:Boolean(raw.targetPage.loaded),evidence:[evidence("target-page",raw.targetPage.url||"target",raw.targetPage.loaded?"Target page loaded":"Target page failed to load",raw.targetPage)]});
