...
class MemoryDeliveryStore {
  ...
  begin(id) {
    ...
    this.entries.set(id, "processing");
...
async function defaultRunner(job, { client, token }) {
  const files = await client.listPullRequestFiles({
    ...
    pullNumber: job.pull_request.number
  });
  ...
  return {
    ...
    text: `${preview || "No changed files reported by GitHub."}${suffix}\n\nAttach an isolated Calibration Studio worker to turn this intake check into a behavioral calibration gate.`
  };
}

function createGitHubWebhookProcessor({
  appId,
  privateKeyPem,
  webhookSecret,
  ...
  if (typeof runner !== "function") throw new Error("GitHub calibration runner must be a function");
  ...
  return async function processWebhook({ headers, rawBody }) {
    ...
    if (!verifyWebhookSignature({ secret: webhookSecret, rawBody, signature })) {
      throw new WebhookError("Invalid GitHub webhook signature", 401);
    }
    ...
    try {
      ...
      if (!job) {
        ...
        return { status: "ignored", delivery_id: deliveryId, event: eventName };
      }
      ...
      const check = await client.createCheckRun({
        ...
        output: { title: "Calibration intake accepted", summary: `Processing pull request #${job.pull_request.number}.` }
      });
      if (!check?.id) throw new Error("GitHub did not return a check-run ID");

      let result;
      try {
        result = await runner(job, { client, token, checkRunId: check.id });
      } catch (error) {
        await client.updateCheckRun({
          ...
          checkRunId: check.id,
          conclusion: "failure",
          output: { title: "Calibration worker failed", summary: String(error?.message || "Calibration worker failed").slice(0, 65535) }
        });
        deliveryStore.complete(deliveryId);
