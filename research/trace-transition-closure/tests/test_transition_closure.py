...
def base_observation():
    c = base_context()
    return {
        "type": "ObservedTransition/0.3",
        "decision_id": c["decision_id"], "action_digest": c["action_digest"],
        ...
        "observed_effects": ["ledger-debit", "ledger-credit"], "executed_at": 1500,
    }


def trace_for(ctx, include_digest=True, duplicate=False, approval=False):
    ref = {"rel":"authorized-intent","id":ctx["decision_id"],"resolver":"https://authority.example"}
    if include_digest: ref["digest"] = digest_object(ctx)
    refs = [ref]
    if duplicate: refs.append(copy.deepcopy(ref))
    if approval:
        refs.append({"rel":"approval-outcome","id":"approval-1",
                     "resolver":"https://authority.example","digest":"sha256:"+"e"*64})
    return {"eat_profile":"tag:agentrust-io.com,2026:trace-v0.2",
            "policy":{"bundle_hash":ctx["policy_digest"]}, "references":refs}


def verified(trace, ctx, obs, **kw):
    return verify_closure(trace, ctx, obs, authorization_evidence_verified=True,
...
class ClosureTests(unittest.TestCase):
    def test_exact_context_passes(self):
        ...
        self.assertEqual(Status.VERIFIED, verify_closure(trace_for(c),c,o,trace_attestation="valid",authorization_evidence_verified=True,observation_evidence_verified=True).closure_status)

    def test_stale_predecessor_fails(self):
        c,o=base_context(),base_observation(); o["predecessor_digest"]="sha256:"+"f"*64
        self.assertIn("STALE_OR_DIFFERENT_PREDECESSOR", verified(trace_for(c),c,o).reasons)

    def test_scope_expansion_fails(self):
        c,o=base_context(),base_observation(); o["touched_resources"].append("account:reserve")
        self.assertIn("RESOURCE_SCOPE_EXPANSION", verified(trace_for(c),c,o).reasons)

    def test_effect_expansion_fails(self):
        c,o=base_context(),base_observation(); o["observed_effects"].append("delete-audit-log")
        self.assertIn("MUTATION_ENVELOPE_EXPANSION", verified(trace_for(c),c,o).reasons)

    def test_expiry_fails(self):
        c,o=base_context(),base_observation(); o["executed_at"]=2001
        self.assertIn("AUTHORIZATION_EXPIRED", verified(trace_for(c),c,o).reasons)

    def test_context_tampering_fails_digest(self):
        c=base_context(); t=trace_for(c); m=copy.deepcopy(c); m["allowed_resources"].append("account:reserve")
    ...
    def test_successor_mismatch_fails(self):
        c,o=base_context(),base_observation(); o["successor_digest"]="sha256:"+"f"*64
    ...
    def test_duplicate_authorized_intent_is_ambiguous(self):
        c=base_context(); r=verified(trace_for(c,duplicate=True),c,base_observation())
    ...
    def test_missing_required_context_cannot_verify(self):
        c=base_context(); c["allowed_resources"]=[]
        r=verified(trace_for(c),c,base_observation())
        self.assertEqual(Status.INDETERMINATE,r.closure_status)

    def test_policy_basis_mismatch_fails(self):
        c=base_context(); t=trace_for(c); t["policy"]["bundle_hash"]="sha256:"+"f"*64
    ...
    def test_missing_trace_policy_is_indeterminate(self):
        c=base_context(); t=trace_for(c); del t["policy"]
    ...
    def test_unverified_observation_cannot_be_promoted_to_verified(self):
        c,o=base_context(),base_observation(); r=verify_closure(trace_for(c),c,o,authorization_evidence_verified=True)
        self.assertEqual(Status.INDETERMINATE,r.closure_status)
    ...
    def test_unverified_authorization_cannot_be_promoted_to_verified(self):
        c,o=base_context(),base_observation(); r=verify_closure(trace_for(c),c,o,observation_evidence_verified=True)
        self.assertEqual(Status.INDETERMINATE,r.closure_status)

    def test_approval_required_missing_is_indeterminate(self):
        c=base_context(); r=verified(trace_for(c),c,base_observation(),require_approval_outcome=True)
        self.assertIn("MISSING_APPROVAL_OUTCOME_REFERENCE",r.reasons)

    def test_approval_required_present_can_verify(self):
        c=base_context(); r=verified(trace_for(c,approval=True),c,base_observation(),require_approval_outcome=True)
        self.assertEqual(Status.VERIFIED,r.closure_status)

    def test_non_ascii_nested_metadata_key_rejected(self):
        c=base_context(); c["metadata"]={"😀":"x"}
        with self.assertRaises(TypeError): digest_object(c)

    def test_float_rejected(self):
        c=base_context(); c["metadata"]={"score":0.5}
        with self.assertRaises(TypeError): digest_object(c)
